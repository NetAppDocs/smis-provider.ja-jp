...
[role="lead"]
ストレージプール（ボリューム）について NetApp SMI-S Provider が報告する管理対象スペースの合計は、SMI-S Provider が WAFL および Snapshotリザーブを含むため、FilerView などの他のストレージ管理ツールで表示されるサイズと異なる場合があります。この差異は想定内の動作であり、是正措置は必要ありません。
